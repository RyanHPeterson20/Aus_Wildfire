...
for (year in 2002:2019) {
  ...
  temp1 <- bounded_pred_df[which(bounded_resp_df$year == year), ]
  start1 <- as.Date(temp1[which(temp1$week == 35), 1])
  end1 <- as.Date(temp1[which(temp1$week == 52), 1])
  
  temp2 <- bounded_pred_df[which(bounded_resp_df$year == year+1), ]
...
NE_ts_df <- data.frame(
  Date = asWeek,
  Residuals = as.vector(t(NE_resids))
)


SE_ts_df <- data.frame(
  Date = asWeek,
  Residuals = as.vector(t(SE_resids))
)
...
all_weeks <- seq(min(SE_ts_df$Date), max(SE_ts_df$Date), by = "week")

# Merge original data with all weeks
full_data <- merge(data.frame(Date = all_weeks), SE_ts_df, by = "Date", all.x = TRUE)

# Fill missing weeks with NA
full_ts <- ts(full_data$Residuals, frequency = 52)

# ACF and PACF on the padded time series
acf(full_ts, na.action = na.pass, main = "ACF with Missing Weeks")
pacf(full_ts, na.action = na.pass, main = "PACF with Missing Weeks")

acf(NE_ts_df$Residuals, main = "ACF without Regular Frequency")
pacf(NE_ts_df$Residuals, main = "PACF without Regular Frequency")

acf(SE_ts_df$Residuals, main = "ACF without Regular Frequency")
pacf(SE_ts_df$Residuals, main = "PACF without Regular Frequency")
```
