...
#TODO: make this
for (year in years) {
  start_date <- as.Date(paste(year, "08-29", sep = "-")) #get week 35
  end_date <- as.Date(paste(year, "12-31", sep = "-")) 
  
  weekly <- seq(from = start_date, to = end_date, by = "week")
  
  # Combine with the main list of dates
  weekly_dates <- c(weekly_dates, weekly)
}

#week(weekly_dates)

#print(weekly_dates)
as.Date(weekly_dates)


week_wednesday <- function(date) {
  # Shift the date so Wednesday becomes the first day of the week
  shifted_date <- date - (wday(date) - 4) %% 7  # `4` corresponds to Wednesday
  
  # Find the week number
  week_num <- as.numeric(difftime(shifted_date, floor_date(shifted_date, "year"), units = "weeks")) + 1
  
  return(week_num)
}

#provides weekly number
floor(week_wednesday(as.Date(weekly_dates)))
```
