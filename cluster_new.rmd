...
SE_coph <- c()
for (i in 1:6) {
  NEhc_man <- constr.hclust(NE_d22, method = hclust_methods[i], chron = TRUE)
  NE_ccc <- cor(NE_d22, cophenetic(NEhc_man))
  NE_coph <- c(NE_coph, NE_ccc)
  
  SEhc_man <- constr.hclust(SE_d22, method = hclust_methods[i], chron = TRUE)
  SE_ccc <- cor(SE_d22, cophenetic(SEhc_man))
