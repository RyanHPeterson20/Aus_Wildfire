...
```{r}
#train data
NEtrain_matrix <- scale(resp_matrix[-c(17:19) ,1:32], center = TRUE, scale = FALSE)
SEtrain_matrix <- scale(resp_matrix[-c(17:19) ,33:64], center = TRUE, scale = FALSE)

#test data
NEtest_matrix <- scale(resp_matrix[17:19 ,1:32], center = TRUE, scale = FALSE)
SEtest_matrix <- scale(resp_matrix[17:19 ,33:64], center = TRUE, scale = FALSE)
...
NEd_train <- NE_train %>% hclust %>% as.dendrogram
NEd_test <- NE_test %>% hclust %>% as.dendrogram
NEd_train_test <- dendlist(train = NEd_train, test = NEd_test)

SEd_train <- SE_train %>% hclust %>% as.dendrogram
SEd_test <- SE_test %>% hclust %>% as.dendrogram
SEd_train_test <- dendlist(train = SEd_train, test = SEd_test)
```

```{r}
Bk_plot(NEd_train, NEd_test, k = 2:16, xlim = c(2,16))
Bk_plot(SEd_train, SEd_test, k = 2:16, xlim = c(2,16))
```
