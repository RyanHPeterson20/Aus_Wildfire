...
#silhouette plots
#TODO: repeat for base, w/o 35-37, w/o 12-14. (total of 8 plots)

#NE AUS, without both
NEhc_test3 <- constr.hclust(NE_d2.3, method = "ward.D2", chron = TRUE)

NEcut_agg3 <- cutree(NEhc_test3, k = 3)

NEagg3_sil <- silhouette(NEcut_agg3, NE_d2.3)
plot(NEagg3_sil, main = "")
pNE <- fviz_silhouette(NEagg3_sil, label = TRUE, print.summary = FALSE)
...
#SE AUS, without both
SEhc_test3 <- constr.hclust(SE_d2.3, method = "ward.D2", chron = TRUE)
SEcut_agg3 <- cutree(SEhc_test3, k = 3) 

SEagg3_sil <- silhouette(SEcut_agg3, SE_d2.3)
plot(SEagg3_sil)
pSE <- fviz_silhouette(SEagg3_sil, label = TRUE, print.summary = FALSE)
...
### New Silhouette Analysis

Repeat above analysis as we remove weeks on either side.
...
#clustering
NEhc_base <- constr.hclust(NE_d2, method = "ward.D2", chron = TRUE)
SEhc_base <- constr.hclust(SE_d2, method = "ward.D2", chron = TRUE)

#groups and silhouette
NEcut_agg3 <- cutree(NEhc_base, k = 3)

NEagg3_sil <- silhouette(NEcut_agg3, NE_d2)
plot(NEagg3_sil, main = "") #keep in for now

NEsil_summary <- summary(NEagg3_sil)
#NEsil_summary$clus.avg.widths

SEcut_agg3 <- cutree(SEhc_base, k = 3)

SEagg3_sil <- silhouette(SEcut_agg3, SE_d2)
plot(SEagg3_sil, main = "") #keep in for now

SEsil_summary <- summary(SEagg3_sil)
#SEsil_summary$clus.avg.widths


#avg_sil(NEbase_matrix)
data.frame(t(NEsil_summary$clus.avg.widths), avg = NEsil_summary$avg.width)
#avg_sil(SEbase_matrix)
SEsil_summary$clus.avg.widths
SEsil_summary$avg.width
```

```{r sil_function}

#fixed for specific hierarchy parameters
#TODO: update to include the avg silhouette
...
```{r iterate_front}
#NEbase_matrix1 <- NEbase_matrix[ ,-1]
#SEbase_matrix1 <- SEbase_matrix[ ,-c(1)]


NEsil_matrix <- as.matrix(t(NEsil_summary$clus.avg.widths), by.row = TRUE)
for (k in 1:6) {
  NEsil_matrix <- rbind(NEsil_matrix, avg_sil( NEbase_matrix[ ,-c(1:k)]))
}

SEsil_matrix <- as.matrix(t(SEsil_summary$clus.avg.widths), by.row = TRUE)
for (j in 1:6) {
...
#THIS is it!
set.panel(1,2)
plot(1:6, diff(NEsil_matrix[,1]), type = "b")
plot(1:6, diff(SEsil_matrix[,1]), type = "b")

#plots of sil coefs

set.panel(1,2)
NEsil_range <- range(NEsil_matrix) 
plot(1:7, NEsil_matrix[,1], type = "b", pch = 15, col = "blue3",
     ylim = NEsil_range)
lines(1:7, NEsil_matrix[,2], type = "b", pch = 15, col = "red3")
lines(1:7, NEsil_matrix[,3], type = "b", pch = 15)

SEsil_range <- range(SEsil_matrix)
plot(1:7, SEsil_matrix[,1], type = "b", pch = 15, col = "blue3",
     ylim = SEsil_range)
lines(1:7, SEsil_matrix[,2], type = "b", pch = 15, col = "red3")
lines(1:7, SEsil_matrix[,3], type = "b", pch = 15)

#update diff plot
set.panel(1,2)
NEdiff_range <- range(diff(NEsil_matrix[,3]),
                      diff(NEsil_matrix[,2]),
                      diff(NEsil_matrix[,1]))
plot(1:6, diff(NEsil_matrix[,3]), type = "b", ylim = NEdiff_range,
     pch = 15)
abline(h = 0, lty = 2)
lines(1:6,  diff(NEsil_matrix[,2]), type = "b", pch = 15, col = "red3")
lines(1:6,  diff(NEsil_matrix[,1]), type = "b", pch = 15, col = "blue3")

SEdiff_range <- range(diff(SEsil_matrix[,3]),
                      diff(SEsil_matrix[,2]),
                      diff(SEsil_matrix[,1]))
plot(1:6, diff(SEsil_matrix[,3]), type = "b", ylim = SEdiff_range,
     pch = 15)
abline(h=0, lty = 2)
lines(1:6,  diff(SEsil_matrix[,2]), type = "b", pch = 15, col = "red3")
lines(1:6,  diff(SEsil_matrix[,1]), type = "b", pch = 15, col = "blue3")
...
#data update without weeks 35-37
NEbase_matrix <- NEbase_matrix[ ,-c(1:3)]
SEbase_matrix <- SEbase_matrix[ ,-c(1:3)]


NEsil_matrix2 <- as.matrix(t(NEsil_summary$clus.avg.widths), by.row = TRUE)
for (i in 1:10) {
...
SEsil_matrix2 <- as.matrix(t(SEsil_summary$clus.avg.widths), by.row = TRUE)
for (i in 1:10) {
...
#sill average
set.panel(1,2)
NEsil_range <- range(NEsil_matrix2) 
plot(1:11, NEsil_matrix2[,3], type = "b", pch = 15, ylim = NEsil_range)
lines(1:11,  NEsil_matrix2[,2], type = "b", col = "red3", pch = 15)
lines(1:11,  NEsil_matrix2[,1], type = "b", col = "blue3", pch = 15)

SEsil_range <- range(SEsil_matrix2) 
plot(1:11, SEsil_matrix2[,3], type = "b", pch = 15, ylim = SEsil_range)
lines(1:11,  SEsil_matrix2[,2], type = "b", col = "red3", pch = 15)
lines(1:11,  SEsil_matrix2[,1], type = "b", col = "blue3", pch = 15)


#diff updates
set.panel(1,2)
NEdiff_range <- range(diff(NEsil_matrix2[,3]),
                      diff(NEsil_matrix2[,2]),
                      diff(NEsil_matrix2[,1]))
plot(1:10, diff(NEsil_matrix2[,3]), type = "b", ylim = NEdiff_range, pch = 15)
abline(h = 0, lty = 2)
lines(1:10, diff(NEsil_matrix2[,2]), type = "b", col = "red3", pch = 15)
lines(1:10, diff(NEsil_matrix2[,1]), type = "b", col = "blue3", pch = 15)

SEdiff_range <- range(diff(SEsil_matrix2[,3]),
                      diff(SEsil_matrix2[,2]),
                      diff(SEsil_matrix2[,1]))
plot(1:10, diff(SEsil_matrix2[,3]), type = "b", ylim = SEdiff_range, pch = 15)
abline(h = 0, lty = 2)
lines(1:10, diff(NEsil_matrix2[,2]), type = "b", col = "red3", pch = 15)
lines(1:10, diff(NEsil_matrix2[,1]), type = "b", col = "blue3", pch = 15)
...
Using MDS to check on the new season boundaries
...
NE_d2.1 <- dist(t(NEbase_matrix1), method = "euclidean")
SE_d2.1 <- dist(t(SEbase_matrix1), method = "euclidean")
...
```{r}
#clustering
NEhc_base <- constr.hclust(NE_d2.1, method = "ward.D2", chron = TRUE)
SEhc_base <- constr.hclust(SE_d2.1, method = "ward.D2", chron = TRUE)
...
```{r}
NEmds <- isoMDS(NE_d2.1, k = 2)
SEmds <- isoMDS(SE_d2.1, k = 2)

NE_color <- rainbow(length(unique(NEcut_agg3)))
SE_color <- rainbow(length(unique(SEcut_agg3)))

season_weeks[28:32]

plot(NEmds$points, pch = 16, col = NE_color[NEcut_agg3], main = "NE MDS", 
     xlab = "Dim 1", ylab = "Dim 2")
#points(NEmds$points[1:3,], cex = 1.5, col = "magenta")
points(NEmds$points[27:29,], cex = 1.5, col = "magenta")
#text(mds_results$points, labels = colnames(NEbase_matrix), cex = 0.8)

plot(SEmds$points, pch = 16, col = SE_color[SEcut_agg3], main = "SE MDS", 
     xlab = "Dim 1", ylab = "Dim 2")
#points(SEmds$points[1:3,], cex = 1.5, col = "magenta")
points(SEmds$points[27:29,], cex = 1.5, col = "magenta")

```


# Histograms and Other Raw Data Plots

Looking at raw data to help make decisions for group 3 (weeks 10-14)
...
hist(NEbase_matrix[,4:12], col = rgb(1, 0, 0, 0.2), breaks = 10, freq = FALSE)
hist(SEbase_matrix[,4:16], col = rgb(1, 0, 0, 0.2), breaks = 10, freq = FALSE)

hist(NEbase_matrix[,18:29], col = rgb(0, 0, 1, 0.2), breaks = 24, freq = FALSE)
hist(SEbase_matrix[,21:29], col = rgb(0, 0, 1, 0.2), breaks = 24, freq = FALSE)


```

```{r}
hist(NEbase_matrix, breaks = 24, freq = FALSE,
     xlab = "Atmospheric CO", main = "", ylim = c(0, 0.10))
hist(NEbase_matrix[,18:29], col = rgb(0, 0, 1, 0.2), breaks = 24, freq = FALSE, add = TRUE)


hist(SEbase_matrix, breaks = 24, freq = FALSE,
     xlab = "Atmospheric CO", main = "", ylim = c(0, 0.12))
hist(SEbase_matrix[,21:29], col = rgb(0, 0, 1, 0.2), breaks = 20, freq = FALSE, add = TRUE)
```

```{r}
hist(NEbase_matrix[ ,18:32], breaks = 24, freq = FALSE,  #col = rgb(0, 0.5, 1, 0.25),
     xlab = "Atmospheric CO", main = "", ylim = c(0, 0.10))
hist(NEbase_matrix[,18:29], col = rgb(0, 0, 1, 0.2), breaks = 24, freq = FALSE, add = TRUE)


hist(SEbase_matrix[ ,21:32], breaks = 24, freq = FALSE, #col = rgb(0, 0.5, 1, 0.25),
     xlab = "Atmospheric CO", main = "", ylim = c(0, 0.12))
hist(SEbase_matrix[,21:29], col = rgb(0, 0, 1, 0.2), breaks = 24, freq = FALSE, add = TRUE)
```
