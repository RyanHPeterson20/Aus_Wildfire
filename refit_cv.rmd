...
for (i in 1:length(seasons)) {
  ...
  for (j in 1:3) {
    ...
    pred.const.interval <- predict(temp.lm.constant, X.valid, interval = "prediction")
    ...
    #assign predictions
    NE.intervals <- rbind(NE.intervals, cbind(y.valid, pred.base.interval, pred.const.interval, pred.vary.interval))
  }
...
for (i in 1:length(seasons)) {
  ...
  for (j in 1:3) {
    ...
    #TODO: create output for the predict!!
    #prediction
...
NErmse.range.noOLR <- range(NE.rmse.noOLR)
SErmse.range.noOLR <- range(SE.rmse.noOLR)
