...
load("predictionTest_functions.R") #predtest functions 
```
...
for (i in 1:length(seasons)) {
  ...
  for (j in 1:3) {
    ...
    #prediction
    y.pred.base <- predict(NE.lms[[j]], X.valid)
    y.pred.const <- predict(temp.lm.constant, X.valid)
    y.pred.vary <- predict(temp.lm.varying, X.valid)
    ...
    NErmse.yearly <- rbind(NErmse.yearly, temp.mat) 
  }
...
for (i in 1:length(seasons)) {
  ...
  for (j in 1:3) {
    ...
    #prediction
    y.pred.base <- predict(SE.lms[[j]], X.valid)
    y.pred.const <- predict(temp.lm.constant, X.valid)
    y.pred.vary <- predict(temp.lm.varying, X.valid)
