...
#full model
NEpreds <- NElag_3group(NE_laglist = NE_iodlag, j = 1:19)
NEresp <- NEresp_3group(NEAus_mat = NEAus_mat, j = 1:19)

SEpreds <- SElag_3group(SE_laglist = SE_iodlag, j = 1:19)
SEresp <- SEresp_3group(SEAus_mat = SEAus_mat, j = 1:19)

#partial model
NEpreds_part <- NElag_3group(NE_laglist = NE_iodlag, j = -c(19))
NEresp_part <- NEresp_3group(NEAus_mat = NEAus_mat, j = -c(19))

SEpreds_part <- SElag_3group(SE_laglist = SE_iodlag, j = -c(19))
SEresp_part <- SEresp_3group(SEAus_mat = SEAus_mat, j = -c(19))
...
```{r group2}
lambda_seq <- seq(400, 120, length.out = 12)
...
SEcoefs2_with <- get_coefs(SE2path_with, max_index = 12, 
                      resp = SEresp[[2]], preds = SEpreds[[2]], 
                      preds_quant = SEpreds[[2]])


SErefit2_with <- refit_bic(SE2path_with, max_index = 12, ebic.gamma = 0.25, gic.gamma = 2.15,
...
```{r test_wo}
lambda_seq <- seq(146, 60, length.out = 15)
...
SEcoefs2_wo <- get_coefs(SE2path_wo, max_index = 15, 
                      resp = SEresp_part[[2]], preds = SEpreds_part[[2]], 
                      preds_quant = SEpreds_part[[2]])


SErefit2_wo <- refit_bic(SE2path_wo, max_index = 15, ebic.gamma = 0.25, gic.gamma = 2.15,
                      lambda.min = FALSE, AIC.c = FALSE,
                      intercept = FALSE,
                      resp = SEresp_part[[2]], preds = SEpreds_part[[2]], 
                      preds_quant =SEpreds_part[[2]])
