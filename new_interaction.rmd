...
#resp
NElogitresp <- NEresp_grouping(NEAus_mat = NEAusmat, j = 1:19)
SElogitresp <- SEresp_grouping(SEAus_mat = SEAusmat, j = 1:19)
```
...
NE_path <- list()
NE_cv <- list()
NE_fit1 <- list() #lambdahat
NE_fit2 <- list() #lambda1se

NE_logitpath <- list()
NE_logitcv <- list()
NE_logitfit1 <- list() #lambdahat
NE_logitfit2 <- list() #lambda1se
...
for (i in 1:n) {
  ...
  NE_fit1[[paste0("Group_", i)]] <- temp_fit1
  NE_fit2[[paste0("Group_", i)]] <- temp_fit2
  ...
  NE_logitpath[[paste0("Group_", i)]] <- logit_path
  NE_logitcv[[paste0("Group_", i)]] <- logit_cv
  NE_logitfit1[[paste0("Group_", i)]] <- logit_fit1
  NE_logitfit2[[paste0("Group_", i)]] <- logit_fit2
}
...
SE_path <- list()
SE_cv <- list()
SE_fit1 <- list() #lambdahat
SE_fit2 <- list() #lambda1se

SE_logitpath <- list()
SE_logitcv <- list()
SE_logitfit1 <- list() #lambdahat
SE_logitfit2 <- list() #lambda1se
...
for (i in 1:n) {
  ...
  SE_fit1[[paste0("Group_", i)]] <- temp_fit1
  SE_fit2[[paste0("Group_", i)]] <- temp_fit2
  ...
  SE_logitpath[[paste0("Group_", i)]] <- logit_path
  SE_logitcv[[paste0("Group_", i)]] <- logit_cv
  SE_logitfit1[[paste0("Group_", i)]] <- logit_fit1
  SE_logitfit2[[paste0("Group_", i)]] <- logit_fit2
}
...
setwd("~/CO_AUS/Aus_CO-main/Interactions")
save(SE_fit1, SE_fit2, SE_cv, SE_path, NE_fit1, NE_fit2, NE_cv, NE_path, file = "hiernet_group.rda")
save(SE_logitfit1, SE_logitfit2, SE_logitcv, SE_logitcv, 
     NE_logitfit1, NE_logitfit2, NE_logitcv, NE_logitcv, file = "hiernet_log.rda")
```
...
plot(x = temp_time, y = temp_resp, pch = 16, 
...
xlim_val <- ymd(c("20010103", "20200401"))
plot(x = temp_time, y = temp_resp, pch = 16, 
     col = "black", xlim = xlim_val, cex = 1.2, ylab = "CO Anomaly", xlab = "",
     main = "NE Aus")
points(x = temp_time, y = NE_hat, pch = 19, 
     col = "magenta3", cex = 0.85)

#test with each group
NE1_resp <- new_resp[which(new_resp$week %in% c(35:46), arr.ind = TRUE), ]
...
main_names <- coef_names[which(main_eff != 0, arr.ind = TRUE)]
coef_main <- main_eff[which(main_eff != 0, arr.ind = TRUE)]

main_df <- data.frame(main_names, coef_main)
...
interact_df <- data.frame(interact_names[,2], interact_names[,1], coef_int )
