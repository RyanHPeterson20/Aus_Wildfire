...
NElogitresp <- NEresp(NEresp_grouping(NEAus_mat = NEAus_mat, j = -c(19)))
SElogitresp <- SEresp(SEresp_grouping(SEAus_mat = SEAus_mat, j = -c(19)))
...
#using only nino/dmi
X_1 <- cbind(as.matrix(NE_preds1[[i]][ ,1:104]),
           as.matrix(NE_preds1_q75[[i]][ ,1:104])  )
...
temp_fit <- hierNet.logistic(X_1, y_logit, lam = temp_cv$lamhat, strong = FALSE, diagonal = TRUE)
```
