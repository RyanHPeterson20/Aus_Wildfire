...
## Newest Response Groups
NE Groups
1. Weeks 35-37;47-52;1-14 Index (1-3, 13-32)
2. Weeks 38-46 Index (4-12)

SE Groups
1. Weeks 35-37; 51-52; 1-14 Index (1-3, 17-32)
2. Weeks 38-50 Index (4-16)
...
#TODO: create new setup section for new groups and adjust the group_function.R
...
## Interactions via hierNet

Work to develop correct interactions. 
-first test: remove indicator function interactions.

```{r test_run}
#test runs for specific interactions
y_1 <- as.numeric(SE_resp[[3]])

dum_mat <- as.matrix(SE_preds_q75[[3]][ ,1:104]) 
dum_mat[,] <- 0
X_int <- cbind(as.matrix(SE_preds[[3]][ ,1:260]), dum_mat)
X_1 <- cbind(as.matrix(SE_preds[[3]][ ,1:260]),
                as.matrix(SE_preds_q75[[3]][ ,1:104]  )) #adjust this as needed for testing

zz_new <- compute.interactions.c(X_int, diagonal=TRUE)
#to adjust this we remove 

#TODO: set up zz matrix before implementing (as above)
#from hierNet()
#  if (is.null(zz)) {
...
#path -> cv
system.time(
  temp_path <- hierNet.path( X_1, y_1, strong = TRUE, diagonal = TRUE, nlam = 50, zz = zz_new)
)
print(temp_path)
...
#temp save 
save(temp_path, temp_cv, file = "temp_hier.rda")
```


```{r}
#testing some functions
test_list <- get_coefs(temp_path, max_index = 50, SE_resp[[3]], preds = SE_preds[[3]], preds_quant = SE_preds_q75[[3]])

#get cv info
lam1se_ind <- which(temp_cv$lamlist == temp_cv$lamhat.1se)
lammin_ind <- which(temp_cv$lamlist == temp_cv$lamhat)
lasso_r2 <- 1-temp_cv$cv.err/var( y_1)
lasso_r2[lam1se_ind]
lasso_r2[lammin_ind]
...
#effect coef
temp_main <- test_list[[lam1se_ind]][[1]]
temp_int <- test_list[[lam1se_ind]][[2]]

temp_main
temp_int

new_coef <- c(temp_main$Coef, temp_int$Coef)

#note: refit_bic currently requires the use of 

#TODO: temporary refit without 2019/2020
remove_year <- 19
SE_resp <- SEresp_grouping(SEAus_mat = SEAus_mat, j = -remove_year)
SE_preds <- SElag_grouping(SE_laglist = SE_laglist_std, j = -remove_year)
SE_preds_q75 <- SElag_grouping(SE_laglist = SE_laglist_q75, j = remove_year)

#create sequence of gamma values.
test_gamma <- seq(0,1,0.05)
eBic_mat <- matrix(NA, ncol = 2)
colnames(eBic_mat) <- c("gamma", "lambda_index")

for (j in 1:length(test_gamma)) {
  temp_list <- refit_bic(temp_path, max_index = 50, ebic.gamma = test_gamma[j],  
                       SE_resp[[3]], preds = SE_preds[[3]], preds_quant = SE_preds_q75[[3]])
  eBic_mat <- rbind(eBic_mat, c(test_gamma[j], which.min(temp_list[[5]][,5])))
}
eBic_mat <- eBic_mat[-1,]
eBic_mat
#basically either 14 or 15


temp_new <- refit_bic(temp_path, max_index = 50, ebic.gamma = 0.25,  
                       SE_resp[[3]], preds = SE_preds[[3]], preds_quant = SE_preds_q75[[3]])
...
summary(temp_new[[1]][[17]])

summary(temp_new[[1]][[14]])

k <- 15
tempridge_cv <- temp_new[[6]]

tempridge_cv[[k]]
which_lambda <- which( tempridge_cv[[k]]$lambda == tempridge_cv[[k]]$lambda.1se)
tempcv_r2 <- 1 - tempridge_cv[[k]]$cvm/var(y_1)
tempcv_r2[99]

#setup coefs for lm and ridge
i <- 15 #lam index
hier_coef <- c(test_list[[i]][[1]]$Coef, test_list[[i]][[2]]$Coef)
lm_coef <- temp_new[[2]][[i]][-1]
ridge_coef <- temp_new[[4]][[i]][-1]

temp_new[[4]][[i]]

#new_lim <- range(hier_coef, lm_coef, ridge_coef)
new_lim <- range(hier_coef, ridge_coef)

plot(1:length(hier_coef), hier_coef, pch = 18, ylim = new_lim)
#points(1:length(lm_coef), lm_coef, pch = 16, col = "magenta3")
points(1:length(ridge_coef), ridge_coef, pch = 17, col = "cyan4")
abline(h = 0, lty = 2)
```

Predictions for SE Aus - Group 3

```{r predictions}
#first: predictions using in-sample
#model setup
SEaus_main <- test_list[[15]][[1]]
SEaus_int <- test_list[[15]][[2]]
...
#prediction section
year <- 19
SE_newresp <- SEresp_grouping(SEAus_mat = SEAus_mat, j = year)
SE_new <- SElag_grouping(SE_laglist = SE_laglist_std, j = year)
SE_new_q75 <- SElag_grouping(SE_laglist = SE_laglist_q75, j = year)

X_test = cbind(as.matrix(SE_new[[3]][ ,1:260]),
            as.matrix(SE_new_q75[[3]][ ,1:104])  )
Xtest_df <- as.data.frame(X_test)

y <- as.numeric(SE_newresp[[3]])
X_temp <- model.matrix(f, Xtest_df)

ridge_pred <- predict(ridge_fit, X_temp, s = ridge_cv$lambda.1se, type = "response")
ridge_pred
y
```


Using code/functions from above to get R^2 and coefs from various lambda values (and plot together.)
-Compare to other fits: (strong/4/no restrictions, weak/4/no restrictions, strong/2/no restrictions)
(note: no restrictions is referring to the inclusion of indicator interactions)

```{r}
#compare with strong/4/no
SEaus_strong <- get_coefs(SEpath_new2$Group_3, max_index = 30, resp = SE_resp[[3]], 
                       preds = SE_preds[[3]], preds_quant = SE_preds_q75[[3]])

SEcv_strong <-  SEcv_new2$Group_3
which(SEcv_strong$lamlist == SEcv_strong$lamhat.1se)
which(SEcv_strong$lamlist == SEcv_strong$lamhat)
SEcv_strong$lamhat.1se
1-SEcv_strong$cv.err/var( y_1)

SEaus_strong[[10]][[1]]
SEaus_strong[[10]][[2]]

SEaus_refit <- refit_bic(SEpath_new2$Group_3, max_index = 30, ebic.gamma = 0.5, group_num = 3, NEAus = FALSE)
SEaus_refit[[5]]

summary(SEaus_refit[[1]]$LamIndex_9)
SEridge_cv <- SEaus_refit[[6]]
which( SEridge_cv$LamIndex_9$lambda == SEridge_cv$LamIndex_9$lambda.1se)
fullcv_r2 <- 1-SEridge_cv$LamIndex_9$cvm/var(y_1)
fullcv_r2[87]

#note: lam_ind 8 (for 1se) 9 (for eBic)
i <- 9 #lam index
hier_coef <- c(SEaus_strong[[i]][[1]]$Coef, SEaus_strong[[i]][[2]]$Coef)
lm_coef <- SEaus_refit[[2]][[i]][-1]
ridge_coef <- SEaus_refit[[4]][[i]][-1]
...
#repeat for weak hierarchy
SEaus_weak <- get_coefs(SE1_path$Group_3, max_index = 20, resp = SE_resp[[3]], 
                       preds = SE_preds[[3]], preds_quant = SE_preds_q75[[3]])
SEcv_weak <- SE1_cv$Group_3

which(SEcv_weak$lamlist == SEcv_weak$lamhat.1se)
which(SEcv_weak$lamlist == SEcv_weak$lamhat)
SEcv_weak$lamhat.1se
1-SEcv_weak$cv.err/var( y_1)

i <- 6
SEaus_weak[[i]][[1]]
SEaus_weak[[i]][[2]]

SEweak_refit <- refit_bic(SE1_path$Group_3, max_index = 20, ebic.gamma = 0.25, group_num = 3, NEAus = FALSE)
SEweak_refit[[5]]

summary(SEweak_refit[[1]]$LamIndex_6)
#ridge r2
SEridge_cv <- SEweak_refit[[6]]
which( SEridge_cv$LamIndex_6$lambda == SEridge_cv$LamIndex_6$lambda.1se)
weakcv_r2 <- 1-SEridge_cv$LamIndex_6$cvm/var(y_1)
weakcv_r2[80]

#plot
i <- 6 #lam index
hier_coef <- c(SEaus_weak[[i]][[1]]$Coef, SEaus_weak[[i]][[2]]$Coef)
lm_coef <- SEweak_refit[[2]][[i]][-1]
ridge_coef <- SEweak_refit[[4]][[i]][-1]
ridge_coef
new_lim <- range(hier_coef, lm_coef, ridge_coef)
...
##try strong-split (group 2 is equiv of group 3/4 )
y_1 <- as.numeric(SE_resp1[[2]])

SEstrong_split <- get_coefs(SE1_path$Group_2, max_index = 20, resp = SE_resp1[[2]], 
                       preds = SE_preds1[[2]], preds_quant = SE_preds1_q75[[2]])

SEstrong_refit <- refit_bic(SE1_path$Group_2, max_index = 20, ebic.gamma = 0.5, 
                            resp = SE_resp1[[2]], 
                            preds = SE_preds1[[2]], preds_quant = SE_preds1_q75[[2]])

SEcv_split <- SE1_cv$Group_2

which(SEcv_split$lamlist == SEcv_split$lamhat.1se)
which(SEcv_split$lamlist == SEcv_split$lamhat)
SEcv_split$lamhat.1se
1-SEcv_split$cv.err/var( y_1)
...
summary(SEstrong_refit[[1]]$LamIndex_4)
#ridge r2
SEridge_cv <- SEstrong_refit[[6]]
which( SEridge_cv$LamIndex_4$lambda == SEridge_cv$LamIndex_4$lambda.1se)
strongcv_r2 <- 1-SEridge_cv$LamIndex_4$cvm/var(y_1)
strongcv_r2[62]

hier_coef <- c(SEstrong_split[[i]][[1]]$Coef, SEstrong_split[[i]][[2]]$Coef)
lm_coef <- SEstrong_refit[[2]][[i]][-1]
ridge_coef <- SEstrong_refit[[4]][[i]][-1]
ridge_coef
new_lim <- range(hier_coef, lm_coef, ridge_coef)
...
output <- foreach(j = 1:length(full_resp)) %dopar% {
  y_1 <- as.numeric(full_resp[[j]])

  dum_mat <- as.matrix(full_preds_q75[[j]][ ,1:104]) 
  dum_mat[,] <- 0
  
  X_int <- cbind(as.matrix(full_preds[[j]][ ,1:260]), dum_mat)
  X_1 <- cbind(as.matrix(full_preds[[j]][ ,1:260]),
                  as.matrix(full_preds_q75[[j]][ ,1:104]  )) 
  
  zz_new <- hierNet::compute.interactions.c(X_int, diagonal=TRUE)
  
  temp_path <- hierNet::hierNet.path( X_1, y_1, strong = TRUE, 
                             diagonal = TRUE, nlam = 50, 
                             zz = zz_new, trace = 0) 
  temp_cv <- hierNet::hierNet.cv(temp_path, X_1, y_1, nfolds = 5)
  
  list(hierpath = temp_path, hiercv = temp_cv)  
}

parallel::stopCluster(cl = my.cluster)  

setwd("~/CO_AUS/Aus_CO-main/Interactions")
save(output, file = "full_strong.rda")
```
...
setwd("~/CO_AUS/Aus_CO-main/Interactions")
save(output, file = "main_strong.rda")
```
...
part_resp <- c(NE_resp, SE_resp)
part_preds <- c(NE_preds, SE_preds)
```


```{r base_parallel}
# 'baseline' (that is: w/o 2019/2020 season)
n.cores <- 8
...
output <- foreach(j = 1:length(part_resp)) %dopar% {
  y_1 <- as.numeric(part_resp[[j]])


  X_1 <- cbind(as.matrix(part_preds[[j]][ ,1:260]))
...
setwd("~/CO_AUS/Aus_CO-main/Interactions")
save(output, file = "base_strong.rda")
```
...
output <- foreach(j = 1:length(full_resp)) %dopar% {
  ...
  temp_path <- hierNet::hierNet.path( X_1, y_1, strong = TRUE, delta = 0.1,
                             diagonal = TRUE, nlam = 50, trace = 0) 
  temp_cv <- hierNet::hierNet.cv(temp_path, X_1, y_1, nfolds = 5)
...
setwd("~/CO_AUS/Aus_CO-main/Interactions")
save(output, file = "enet_strong.rda")
```
...
output <- foreach(j = 1:length(full_resp)) %dopar% {
  ...
  temp_path <- hierNet::hierNet.path( X_1, y_1, strong = TRUE, delta = 0.25,
                             diagonal = TRUE, nlam = 50, trace = 0) 
  temp_cv <- hierNet::hierNet.cv(temp_path, X_1, y_1, nfolds = 5)
...
setwd("~/CO_AUS/Aus_CO-main/Interactions")
save(output, file = "enet25_strong.rda")
```
...
setwd("~/CO_AUS/Aus_CO-main/Interactions")
save(output, file = "group_strong.rda")
```



```{r twogroup_parallel_sans19}
#part model
NE_preds <- NElag_grouping(NE_laglist = NE_laglist_std, j = -c(19))
NE_resp <- NEresp_grouping(NEAus_mat = NEAus_mat, j = -c(19))
...
setwd("~/CO_AUS/Aus_CO-main/Interactions")
save(output, file = "group_basestrong.rda")
...
Quick test for elastic net
(all of this is meh, not great)
```{r}
tic()
j <- 2 #NE aus group 2
y_1 <- as.numeric(full_resp[[j]])
...
temp_path <- hierNet::hierNet.path( X_1, y_1, strong = TRUE, delta = 0.05,
                             diagonal = TRUE, nlam = 25, trace = 0) 
toc()

#temp_cv <- hierNet::hierNet.cv(temp_path, X_1, y_1, nfolds = 10)

refit_test <- refit_bic(temp_path, max_index = 25, ebic.gamma = 0.25, lambda.min = TRUE, AIC.c = FALSE,
                      resp = y_1, preds = X_1, preds_quant = NE_preds_q75[[2]])

refit_test[[5]]

which.min(refit_test[[5]][,2])
which.min(refit_test[[5]][,6])
which.min(refit_test[[5]][,9])

i <- 3
ridge_coef <- refit_test[[4]][[i]][-1]
ridge_coef
length(ridge_coef)
```
...
temp_path3 <- hierNet::hierNet.path( X_1, y_1, strong = TRUE, delta = 0.15,
                             diagonal = TRUE, nlam = 25, trace = 0) 
toc()

temp_cv <- hierNet::hierNet.cv(temp_path3, X_1, y_1, nfolds = 5)

plot(temp_cv)

which(temp_cv$lamlist == temp_cv$lamhat.1se) 

refit_test3 <- refit_bic(temp_path3, max_index = 25, ebic.gamma = 0.25, lambda.min = FALSE, AIC.c = FALSE,
                      resp = y_1, preds = X_1, preds_quant = NE_preds_q75[[3]])

refit_test3[[5]]

which.min(refit_test3[[5]][,2])
which.min(refit_test3[[5]][,6])
which.min(refit_test3[[5]][,9])

i <- 9
ridge_coef <- refit_test3[[4]][[i]][-1]
ridge_coef
length(ridge_coef)
...
```{r}
#re-running 4 group models with strong hierarchy and increased 
...
for (i in 3:4) {
  ...
  #path -> cv -> fit
  temp_path <- hierNet.path( X_1, y_1, strong = TRUE, diagonal = TRUE, nlam = 30)
  temp_cv <- hierNet.cv(temp_path, X_1, y_1, nfolds = 5)

  
  SEpath_new[[paste0("Group_", i)]] <- temp_path
  SEcv_new[[paste0("Group_", i)]] <- temp_cv
  #SE1_fit1[[paste0("Group_", i)]] <- temp_fit1
  #SE1_fit2[[paste0("Group_", i)]] <- temp_fit2
...
#setwd("~/CO_AUS/Aus_CO-main/Interactions")
#save(SEpath_new, SEcv_new, file = "SEAus_hiernet_temp.rda")

SEpath_new2 <- SEpath_new
SEcv_new2 <- SEcv_new
save(SEpath_new2, SEcv_new2, file = "SEAus_hiernet_temp2.rda")
...
for (i in 1:4) {
  ...
  NEpath_new[[paste0("Group_", i)]] <- temp_path
  NEcv_new[[paste0("Group_", i)]] <- temp_cv
  #SE1_fit1[[paste0("Group_", i)]] <- temp_fit1
  #SE1_fit2[[paste0("Group_", i)]] <- temp_fit2
...
save(NEpath_new, NEcv_new, file = "NEAus_hiernet_strong.rda")
```
...
```{r}
#perform fits on SE Aus
i <- 3
y_1 <- as.numeric(SE_resp[[i]])
X_1 <- cbind(as.matrix(SE_preds[[i]][ ,1:260]),
                as.matrix(SE_preds_q75[[i]][ ,1:104]  ))

SEfit3_min <- hierNet(X_1, y_1, lam = SEcv_new2[[1]]$lamhat, strong = TRUE, diagonal = TRUE)
```
...
SEfit2_new <- hierNet(X_1, y_1, lam = SEcv_new[[i]]$lamhat.1se, strong = TRUE, diagonal = TRUE)
```
...
png(filename = "the_season.png", width = 2200, height = 1800, res = 200)
set.panel(2,1)
...
text_atnew <- c(2, 51.6, 55.5, 103.6, 108.5, 155.6, 160.5, 207.6, 212.5, 261)
label_new <- rep(c(1,52), 5)
...
ycoef <- range(coef_main, coef1_main)

setwd("~/CO_AUS/Aus_CO-main/Interactions")
  
png(filename = "NEAus_coef3.png", width = 2400, height = 1100, res = 200)
par(mar = c(3, 4, 4, 7.5) + 0.25)
plot(main_loc, coef_main, pch = 16, xlim = c(0,260), ylim = ycoef,
     main = "NE Aus : Group 3",
     ylab = "Coefficients", xlab = "", xaxt = "n", cex.main = 1.5, cex = 1.25)
points(main1_loc, coef1_main, pch = 16, col = "magenta3", cex = 1.25)
abline(h=0, lty =2)
abline(v =c(0.5, 52.5, 104.5, 156.5, 208.5, 260.5), lty = 2, col = "blue3")
...
ticks_new <- c(261, 312, 313, 364)

at_new <- c(261.5, 311.5, 314, 364.5)
labels_new <- rep(c(1,52), 2)

#TODO: make this plot better, like the one from above
setwd("~/CO_AUS/Aus_CO-main/Interactions")
  
png(filename = "NEAus_coef2IND.png", width = 2400, height = 1100, res = 200)
par(mar = c(4, 4, 3, 7.5) + 0.25)
plot(main_loc[20:25], coef_main[20:25], pch = 16, xlim = c(261,365), ylim = ycoef,
     ylab = "Coefficients", xlab = "", xaxt = "n", cex = 1.25)
points(main1_loc[15:18], coef1_main[15:18], pch = 16, col = "magenta3", cex = 1.25)
abline(h=0, lty =2)
abline(v = c(260.5, 312.5, 364.5), lty = 2, col = "blue3")
axis(1, at = ticks_new, labels = FALSE)
text(at_new, par("usr")[3]-0.15, labels = labels_new, xpd = TRUE, adj = 1, cex = 0.75)
text(x = c(290, 342),
     y =par("usr")[3]-0.25,
     labels = ind_exp, 
     xpd = TRUE, adj = 1)
text(x = 258.5, par("usr")[3]-0.155, labels = c("Lag"), xpd = TRUE, adj = 1, cex = 0.75)
legend("topright", inset = c(-0.12, 0),
       legend = c("2001-2020", "2001-2019"),
       col = c("black", "magenta3"),
       pch = c(16,16),
...
setwd("~/CO_AUS/Aus_CO-main/Interactions")
ggsave("SE_in2.png", plot = table_plot, width = 7, height = 8, dpi = 300)
ggsave("SE_out2.png", plot = table_plotout, width = 7, height = 7, dpi = 300)
```
...
for (i in 1:n) {
  ...
  #path -> cv -> fit
  temp_path <- hierNet.path( X_1, y_1, strong = TRUE, diagonal = TRUE)
  temp_cv <- hierNet.cv(temp_path, X_1, y_1, nfolds = 5)

  temp_fit1 <- hierNet(X_1, y_1, lam = temp_cv$lamhat, strong = TRUE, diagonal = TRUE)
  temp_fit2 <- hierNet(X_1, y_1, lam = temp_cv$lamhat.1se, strong = TRUE, diagonal = TRUE)
...
setwd("~/CO_AUS/Aus_CO-main/Interactions")
save(SE1_path, SE1_cv, SE1_fit1, SE1_fit2, NE1_path, NE1_cv, NE1_fit1, NE1_fit2, file = "hiernet_group_split.rda")
