...
#rename bounded_pred_df
bounded_pred2_df <- bounded_pred_df
...
for(i in season_weeks){
  ...
  for (j in 1:52) {
    ...
    lag_pred2 <- bounded_pred2_df[which(bounded_pred2_df$week == lag_week), ]
    lag_pred2 <- lag_pred2[which(lag_pred2$year >= lag_year[1] & lag_pred2$year <= lag_year[2]), ]
    ...
    dmi_lag <- cbind(dmi_lag, lag_pred2$dmi.anomaly)
    wtio_lag <- cbind(wtio_lag, lag_pred2$wtio.anomaly)
    etio_lag <- cbind(etio_lag, lag_pred2$etio.anomaly)
  ...
  wtio_lag <- wtio_lag[,-1]
  colnames(wtio_lag) <- paste0("wtio_lag", lag_vec[1:52])
  wtio_lag <- scale(wtio_lag, center = TRUE, scale = TRUE)
  
  etio_lag <- etio_lag[,-1]
  colnames(etio_lag) <- paste0("etio_lag", lag_vec[1:52])
  etio_lag <- scale(etio_lag, center = TRUE, scale = TRUE)
  ...
  NEweek_lag <- data.frame(tempNE_df, nino_lag, wtio_lag, etio_lag, tsa_lag, aao_lag, NEolr_lag)
  SEweek_lag <- data.frame(tempSE_df, nino_lag, wtio_lag, etio_lag, tsa_lag, aao_lag, SEolr_lag)
  
  NE_laglist_std[[paste("Week ", i)]] <- NEweek_lag
  SE_laglist_std[[paste("Week ", i)]] <- SEweek_lag
}

#rename laglist_std (and save)
NE_iodlag <- NE_laglist_std
SE_iodlag <- SE_laglist_std


setwd("~/CO_AUS/Aus_CO-main/Interactions_New")

save(NE_iodlag, SE_iodlag, bounded_pred2_df, file = "IOD_lag.rda")
```
