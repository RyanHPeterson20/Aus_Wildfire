...
#TODO: interpolate to 1x1 grid for smaller indian ocean grid
#set-up coordinate matrix, and vector of observations


#TODO: compare the fine grid and interpolated visually
...
```{r test.figures}
z_temp <- range(sst.array, na.rm = TRUE)

image.plot(list(x = lon.values, y = rev(lat.values), z = sst.array[,,1]), col = tim.colors(256), zlim = z_temp, 
           xlab = "Lon", ylab = "Lat", main = paste0("GODAS: ", format(as.Date(times[2]), "%B %Y")))
world(add=TRUE)

image.plot(list(x = lon.values, y = rev(lat.values), z = test_sst.oisst), col = tim.colors(256), zlim = z_temp, 
