...
```{r temp_function}
make_weights <- function(lat_deg, take_sqrt = TRUE) {
  stopifnot(is.numeric(lat_deg))
  # convert to radians
  lat_rad <- lat_deg * pi / 180
  # numerical guard: cos(±90°) ~ 0; tiny negatives from rounding -> clamp to 0
  w <- pmax(cos(lat_rad), 0)
  if (take_sqrt) w <- sqrt(w)
  # if someone passed impossible lats, mark NA
  w[abs(lat_deg) > 90] <- NA_real_
  return(w)
}
```
...
#scale sst.full.avg
sst.full.scale <- apply(sst.full.avg, 2, scale, center = FALSE, scale = TRUE)
...
#TODO: attempt with areal weghts
wlat <- make_weights(lat.values.IOD)
...
#for (j in 1:nt) {
#  sst.anom.pIOD[j, ,] <- sweep(sst.anom.pIOD[j, ,], 2, wlat, `*`)
#}
...
PC.iod <- pca.pIOD$PC
apply(PC.iod, 2, sd, na.rm = TRUE)

PC.unit <- sweep(PC.iod, 2, apply(PC.iod, 2, sd, na.rm = TRUE), "/")

#pc.std.IOD <- scale(pca.pIOD$PC, center = FALSE, scale = TRUE)

#pc.std.IOD[,1] <- scale(pca.pIOD$PC[,1], center = FALSE, scale = TRUE)
#pc.std.IOD[,2] <- scale(pca.pIOD$PC[,2], center = FALSE, scale = TRUE)
...
#add in overlay output


#transparent saves
setwd("~/CO_AUS/Aus_CO-main/pIOD/Figures")

png("PCoverlay_new2.png", width = 1500, height = 1500, bg = "transparent", type = "cairo") 
...
```{r}
pc.alt <- sst.eigen(sst.anom.pIOD)

pc.std.alt <- scale(pc.alt$PC, center = FALSE, scale = TRUE)
...
Updated work with better grid of data 

```{r multi_prod}
sst.full.avg <- (sst.GODAS.anom2 + sst.ORAS.anom2 +
                 sst.OISST.anom2 + sst.SODA.anom2 )/4


#multi-product averages
mat_list <- list(sst.GODAS.anom2, sst.ORAS.anom2,
...
```{r }
sst.anom.pIOD <- sst.full.avg[lon.range.IOD[1]:lon.range.IOD[2], lat.range.IOD[1]:lat.range.IOD[2], ]

dim(sst.anom.pIOD) #as [nx, ny, nt]
sst.anom.new <- aperm(sst.anom.pIOD,  perm = c(3, 2, 1))

pca.new <- sst.eof(sst.anom.new, kmode = 2)


#pc.std.IOD <- scale(pca.new$PC, center = FALSE, scale = TRUE)

pc.std.IOD[,1] <- scale(pca.new$PC[,1], center = FALSE, scale = TRUE)
pc.std.IOD[,2] <- scale(pca.new$PC[,2], center = FALSE, scale = TRUE)
...
pca.anom <- sst.eof(sst.anom, kmode = 2)

setwd("~/CO_AUS/Aus_CO-main/pIOD")
save(pca.pIOD, file = "pca_newest.rda")

pc.std.IOD <- scale(pca.anom$PC, center = TRUE, scale = TRUE)
...
png("pIOD_overlay.png", width = 2000, height = 1200, bg = "transparent", type = "cairo") 
par(mar = c(0,0,0,0), oma = c(0,0,0,0), xaxs = "i", yaxs = "i")
...
png("PCoverlay_interp3.png", width = 1500, height = 1500, bg = "transparent", type = "cairo") 
par(mar = c(0,0,0,0), oma = c(0,0,0,0), xaxs = "i", yaxs = "i")
plot(PC1, PC2, pch = 21, col = "black",
     bg = alpha("cyan",.65), xlim = c(-2,4), ylim = c(-3, 5),
     xlab = "PC1", ylab = "PC2", cex = 5)
...
sst.2019.iod <- sst.2019.godas[lon.range.IOD[1]:lon.range.IOD[2], lat.range.IOD[1]:lat.range.IOD[2]]
...
godas.amom.iod <- GODAS.anom2[lon.range.IOD[1]:lon.range.IOD[2], lat.range.IOD[1]:lat.range.IOD[2], 35]
...
## EOFs/Spatial Pattern

```{r smaller_piod}
#just over the tropical IOD
EOF1 <- matrix(pca.pIOD$EOF[,1], nrow = 11, ncol = 61)
EOF2 <- matrix(pca.pIOD$EOF[,2], nrow = 11, ncol = 61)


imagePlot(list(x = lon.values.IOD, y = lat.values.IOD, z = -t(EOF1)))
world(add = TRUE)

imagePlot(list(x = lon.values.IOD, y = lat.values.IOD, z = t(EOF2)))
world(add = TRUE)


s.eof <- (-t(EOF1) + t(EOF2))/sqrt(2)
m.eof <- (-t(EOF1) - t(EOF2))/sqrt(2)

pIOD.range <- range(s.eof, m.eof, na.rm = TRUE)
pIOD.absmax <- max(abs(pIOD.range))
pIOD.range <- c(-pIOD.absmax, pIOD.absmax)
#default breaks
cols.ryb <- rev(colorRampPalette(brewer.pal(11, "RdYlBu"))(12))
breaks.dumb <- c(pIOD.range[1], seq(-1, 1, 0.2), pIOD.range[2])
leg.brks2 <- seq(-1.2, 1.2, 0.2) 


imagePlot(list(x = lon.values.IOD, y = lat.values.IOD, z = s.eof),
          col = cols.ryb, breaks = breaks.dumb, zlim = pIOD.range, 
           xlab = "Lon", ylab = "Lat",
           axes = FALSE)
...
imagePlot(list(x = lon.values.IOD, y = lat.values.IOD, z = m.eof),
          col = cols.ryb, breaks = breaks.dumb, zlim = pIOD.range, 
           xlab = "Lon", ylab = "Lat",
           axes = FALSE)
axis(1, at = c(60, 90, 120))  
...
son.mean <- matrix(mean_sst, nrow =  ny * nx, ncol = nt)

pc1.son <- son.mean %*% PC1
pc2.son <- son.mean %*% PC2

eof1.son <- matrix(pc1.son/nt, nrow = nx, ncol = ny)
eof2.son <- matrix(pc2.son/nt, nrow = nx, ncol = ny)
...
#extended fig1a
png(filename = "eof1.png", width = 2400, height = 1600, res = 250)
...
png(filename = "eof2.png", width = 2400, height = 1600, res = 250)
par(mar = c(7, 5, 2, 5), mgp = c(2.25, 1, 0))
...
```{r}
s.eof <- (eof1.son + eof2.son)/sqrt(2)
m.eof <- (eof1.son - eof2.son)/sqrt(2)

#sindex.son <- son.mean %*% s.index
#mindex.son <- son.mean %*% m.index

#s.eof <- matrix(sindex.son, nrow = nx, ncol = ny)
#m.eof <- matrix(mindex.son, nrow = nx, ncol = ny)
...
pIOD.range <- c(-pIOD.absmax, pIOD.absmax)
#default breaks
cols.ryb <- rev(colorRampPalette(brewer.pal(11, "RdYlBu"))(18))
breaks.new <- seq(pIOD.range[1], pIOD.range[2], length.out = length(cols.ryb)+1)
...
par(mar = c(4, 5, 2, 5), mgp = c(2.25, 1, 0))
imagePlot(list(x = lon.wide, y = lat.wide, z = s.eof), 
           col = cols.ryb, breaks = breaks.new, zlim = pIOD.range, 
           xlab = "Lon", ylab = "Lat",
           axes = FALSE)
axis(1, at = c(60, 90, 120))  
axis(2)                      
...
par(mar = c(4, 5, 2, 5), mgp = c(2.25, 1, 0))
imagePlot(list(x = lon.wide, y = lat.wide, z = m.eof), 
           col = cols.ryb, breaks = breaks.new, zlim = pIOD.range, 
           xlab = "Lon", ylab = "Lat",
           axes = FALSE)
axis(1, at = c(60, 90, 120))  
