...
#test block for internals of for loops
k <- 1 #week (season_weeks[k])
...
#get coeffs
#TODO: set group values
j <- 1 #group 1 (since week 1 is in group 1)
temp_fuse <- NEfuse_all[[i]][[j]]

#TODO: get lambda from temp_fuse
...
```{r }
#out-of sample weekly resids
#TODO: set-up code for out of sample checks
#uses NEfuse_all and SEfuse_all
#group (weekly setup) refer to AGU_viz.rmd
...
#repeat the weekly data using NEbase_matrix and SEbase_matrix
#by year
for (i in 1:18) {

  temp_fuse <- NEfuse_all[[i]]
  
  for (j in 1:length(temp_fuse)) {
    temp_resp <- NE_resp[[j]]
    temp_pred <- as.matrix(NE_preds[[j]])
    
    temp_group <- temp_fuse[[j]]
    temp_coef <- coef()
  }
}
