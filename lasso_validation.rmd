...
for (j in 1:length(SEfuse_grouplist)) {
  ...
  SE1_coefs <- coef(SEfuse_grouplist[[j]], lambda = SEfuse_cv[[j]]$lambda.min)$beta
...
for (j in 1:length(NEfuse_grouplist)) {
  ...
  NE1_coefs <- coef(NEfuse_grouplist[[j]], lambda = NEfuse_cv[[j]]$lambda.min)$beta
...
boxplot(SE_resids, main = "SE Aus Residuals", 
        xlab = "Group", ylab = "", pch = 16)
boxplot(NE_resids, main = "NE Aus Residuals", 
        xlab = "Group", ylab = "", pch = 16)
```


```{r outofsample}
#out-of sample resids
#TODO: set-up code for out of sample checks
#uses NEfuse_all and SEfuse_all

#by year first
testyear <- NEfuse_all[[1]]
testyear[[1]]
