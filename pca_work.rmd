...
```{r pca_check}
pca_nino <- prcomp(nino_df, center = TRUE, scale. = TRUE, retx = TRUE)
...
plot(pca_nino$x[,1], pca_nino$x[,2], xlim = c(-10, 20),
     xlab = "PC1", ylab = "PC2", main = "PCA Nino", pch=16)
text(pca_nino$x[,1], pca_nino$x[,2], labels = colnames(nino_df), pos = 4)
```
