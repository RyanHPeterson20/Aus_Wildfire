...
#TODO: finalize ORAS for plotting and further manipulation
lat.bound <- which(get.lat.new <= wide_maxLat & get.lat.new >= wide_minLat, arr.ind = TRUE)
lon.bound <- which(get.lon.new <= wide_maxLon & get.lon.new >= wide_minLon, arr.ind = TRUE)


min(lat.bound[,c(1,2)])
lat.bound[max(lat.bound[,c(1,2)]), ]

osar_bounded <- which(get.lat.new <= wide_maxLat & get.lat.new >= wide_minLat & get.lon.new <= wide_maxLon & get.lon.new >= wide_minLon)

lat.bound.new <- get.lat.new[osar_bounded]
lon.bound.new <- get.lon.new[osar_bounded]

#TODO: convert to matrix
sst.osar.new <- as.matrix(sst.osar[osar_bounded])

range(sst.osar.new, na.rm = TRUE)
