...
```{r}
#TODO: perform corrections on to somehow standardize our data, especially to make sure
## that we only standardize the training data.

#TODO: ensure that "new" data (testing sets) are also standardized. 
...
TODO: add in a 1 column into the data for an intercept and create the appropriate 1 in the D matrix.
...
ne_resid <-  as.data.frame(matrix(NA, ncol = 4))
colnames(ne_resid) <- c("Week", "True", "Pred", "Resid")

#ne group1
ne_beta1 <- NEgroup1_coef$beta
for (k in 1:3) {
  ne_week <- NE_laglist[[k]]
  temp_preds <- as.matrix(ne_week[19, 3:210])
  temp_resp <- ne_week[19, 2]
    
  pred_temp <- temp_preds %*% ne_beta1
  temp_resid <- temp_resp - pred_temp
  ne_resid <- rbind(ne_resid, c(season_weeks[k], temp_resp, pred_temp, temp_resid))
}
...
se_resid <-  as.data.frame(matrix(NA, ncol = 4))
colnames(se_resid) <- c("Week", "True", "Pred", "Resid")
...
for (k in 1:3) {
  ...
  pred_temp <- temp_preds %*% se_beta1
  temp_resid <- temp_resp - pred_temp
  se_resid <- rbind(se_resid, c(season_weeks[k], temp_resp, pred_temp, temp_resid))
}
...
```{r plot}
#TODO: update notation of plot add other features
plot(1:32, ne_resid$Resid, type = "l", main = "NE Aus Residuals", xaxt = "n", 
     ylab = "Residual", xlab = "Weeks")
lines(1:32, ne_resid$True, lwd = 1.5, col = "blue")
lines(1:32, ne_resid$Pred, lwd = 1.5, col = "magenta")
axis(1, at=1:32, labels = season_weeks)


plot(1:32, se_resid$Resid, type = "l", main = "SE Aus Residuals", xaxt = "n", 
     ylab = "Residual", xlab = "Weeks")
lines(1:32, se_resid$True, lwd = 1.5, col = "blue")
lines(1:32, se_resid$Pred, lwd = 1.5, col = "magenta")
axis(1, at=1:32, labels = season_weeks)
```

```{r residual_pred}
#NE Aus: residual spikes in group 5 (52, 1-3) [18-21]
#SE Aus: residual spikes in group 4 (51-52, 1-2) [17-20]

#get non-zero coeffs, then associate with list of lag predictors from each group, the see if any are in the top n.
##Or get a dist of those lag values to see if they are exceding some value.

#ne group 5

#ne week 52
season_weeks[18]
NE_test <- NE_laglist[[18]]
temp_NE52 <- NE_test[, 3:210]

ne_index <- which(NEgroup5_coef$beta != 0)

ne52_lag <- temp_NE52[,ne_index]
ne52_names <- colnames(ne52_lag)
ne52_mean <- sapply(ne52_lag, mean)
ne52_sd <- sapply(ne52_lag, sd)

#scaling factor:
c <- 1.75
ne52_over <- ne52_mean + (c * ne52_sd)
ne52_under <- ne52_mean - (c * ne52_sd)

n <- length(ne52_lag)
NEover_list <- list()
NEunder_list <- list()
for (i in 1:n) {
  over <- which(ne52_lag[,i] >= ne52_over[i])
  
  NEover_list[[paste0(ne52_names[i])]] <- season_years[over]
  
  under <- which(ne52_lag[,i] <= ne52_under[i])
  
  NEunder_list[[paste0(ne52_names[i])]] <- season_years[under]
}
...
#se week 51
season_weeks[17]
SE_test <- SE_laglist[[17]]
temp_SE51 <- SE_test[, 3:210]
 
se_index <- which(SEgroup4_coef$beta != 0 )

se51_lag <- temp_SE51[,se_index]
se51_names <- colnames(se51_lag)
se51_mean <- sapply(se51_lag, mean)
se51_sd <- sapply(se51_lag, sd)

#scaling factor:
c <- 1.75
se51_over <- se51_mean + (c * se51_sd)
se51_under <- se51_mean - (c * se51_sd)

n <- length(se51_lag)
SEover_list <- list()
SEunder_list <- list()
for (i in 1:n) {
  over <- which(se51_lag[,i] >= se51_over[i])
  
  SEover_list[[paste0(se51_names[i])]] <- season_years[over]
  
  under <- which(se51_lag[,i] <= se51_under[i])
  
  SEunder_list[[paste0(se51_names[i])]] <- season_years[under]
}

#se51_names
#SEgroup4_coef$beta[se_index]
...
add in squared terms for:
SE Aus: dmi lag 1-12
NE Aus: dmi lag 18-20

```{r}
#SE group 4
SE_resp4 <- SE_resp[[4]]
SE_pred4 <- as.matrix(SE_preds[[4]][ ,1:208])
SE_olr4 <- as.matrix(SE_preds[[4]][ ,209:260])  

newSE_pred4 <- (SE_pred4[ ,53:64])^2
#newSE_pred4
SE_resid4 <- c(as.vector(SEgroup4_list[[1]]$Resid), as.vector(SEgroup4_list[[2]]$Resid),
                       as.vector(SEgroup4_list[[3]]$Resid), as.vector(SEgroup4_list[[4]]$Resid))

hist(SE_resid4)

resid_lm <- lm(SE_resid4 ~ newSE_pred4)
resid_lm$coefficients

D_olr <- getD1d(52)
D_sqr <- getD1d(12)
                       
SEfuse_resid4 <- fusedlasso(y = SE_resid4, X = newSE_pred4, D_sqr, gamma = 0.33)

n <- length(SE_resid4)
p <- ncol(newSE_pred4)
lambda1 <- sqrt(n * log(p))

SEresid_4 <- coef(SEfuse_resid4, lambda = lambda1)

plot(1:12, SEresid_4$beta, type = "l", col = "blue")
abline(h = 0, lty = 2)


SE_test <- SE_laglist[[17]]
temp_SE51 <- (SE_test[, 55:66])^2
newSEresid_4 <- as.vector(SEresid_4$beta)

as.matrix(temp_SE51) %*% newSEresid_4
as.vector(SEgroup4_list[[1]]$Resid)
se_resid[17, 4]
```

Notes/Ideas:

From the below work we see that the most comparable dmi to 2019 is 2006, 
we see that there is a very high residual in SE Aus week 51 from 2006. 

Repeat this residual "analysis" for other high residual groups and years.

But we still need to fit something that works for this, we may need to generate some synthetic conditions. 

Convert OLR into something like: weeks above 0 (with buffer) in a given lag.
$$\sum_{i=1}^{p(n)} \mathbb{I} \{x_i \geq c\}.$$

```{r distance}
#get distance from mean for dmi 1-12 in SE Aus 4 (2019) 

test_lag <- SE_laglist[[18]]
dmi_lag <- test_lag[,55:66]
dmi_mean <- sapply(dmi_lag, mean)

diff_sub <- dmi_lag[19, ] - dmi_mean

dmi_full <- test_lag[ ,55:106]
full_mean <- sapply(dmi_full, mean)

diff_full <- dmi_full[19, ] - full_mean
diff_full18 <- dmi_full[18, ] - full_mean
diff_full17 <- dmi_full[17, ] - full_mean
diff_full16 <- dmi_full[16, ] - full_mean
diff_full15 <- dmi_full[15, ] - full_mean
diff_full6 <- dmi_full[6, ] - full_mean

result <- sweep(dmi_full, 2, full_mean, FUN = "-")
range(result)

plot(1:52, diff_full, type = "l", main = "DMI 2019/2020", ylim = range(result))
lines(1:52, diff_full18, col = "red")
lines(1:52, diff_full17, col = "blue")
lines(1:52, diff_full15, col = "darkgreen")
lines(1:52, diff_full6, col = "darkmagenta")
abline( v= c(1,12), lty = 2, col = "red")

#TODO: repeat with OLR

olr_lag <- test_lag[ ,211:262]
olr_mean <- sapply(olr_lag, mean)

diff_olr19 <- olr_lag[19, ]# - olr_mean
#diff_olr18 <- olr_lag[18, ] - olr_mean
#diff_olr6 <- olr_lag[6, ] - olr_mean
diff_olr6 <- olr_lag[6, ]# - olr_mean
diff_olr4 <- olr_lag[4, ]# - olr_mean #"base" condition for normal amounts
diff_olr14 <- olr_lag[14, ]# - olr_mean #min/low atm CO year

plot(1:52, diff_olr19, type = "l", main = "OLR values")
#lines(1:52, diff_olr4, col = "darkgreen")
lines(1:52, diff_olr14, col = "blue")
#lines(1:52, diff_olr6, col = "darkmagenta")
abline(h =0, lty = 2, col = "red")

#TODO: create plots for either the data or residuals

#start with SE residuals (looking for spikes in SE group 4)
SEgroup4_resid <- cbind(as.vector(SEgroup4_list[[1]]$Resid),
                        as.vector(SEgroup4_list[[2]]$Resid),
                        as.vector(SEgroup4_list[[3]]$Resid),
                        as.vector(SEgroup4_list[[4]]$Resid))

#SEgroup4_resid <- rbind(SEgroup4_resid, )

plot(1:4, SEgroup4_resid[18, ], type = "l", col = "red")
lines(1:4, SEgroup4_resid[6, ])

#"raw" data:
segroup4_raw <- bounded_resp_df[which(bounded_resp_df$week == 51), 3]
segroup4_raw <- cbind(segroup4_raw, 
                      bounded_resp_df[which(bounded_resp_df$week == 52), 3])
segroup4_raw <- cbind(segroup4_raw, 
                      c(bounded_resp_df[which(bounded_resp_df$week == 1), 3][-1]))
segroup4_raw <- cbind(segroup4_raw, 
                      c(bounded_resp_df[which(bounded_resp_df$week == 2), 3][-1]))

colnames(segroup4_raw) <- c("51", "52", "1", "2")

segroup4_raw <- cbind(segroup4_raw, rowMeans(segroup4_raw))
...
#17-20 (week 51,52,1,2)
test_lag <- SE_laglist[[17]]
olr_lag <- test_lag[ ,211:262]
lag10 <- c()
lag20 <- c()
#lag25 <- c()
for (i in 1:19) {
  lag10 <- c(lag10, length(which(olr_lag[i,1:10] >= 2)))
  lag20 <- c(lag20, length(which(olr_lag[i,11:20] >= 2)))
  #lag25 <- c(lag30, length(which(olr_lag[i,21:25] >= 2)))
}
...
#plot left and right side, with right side as the week's CO values by year
plot(1:19, lag10, xlab = "year", pch = 16)
